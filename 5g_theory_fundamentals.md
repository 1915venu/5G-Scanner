...
flowchart LR
    subgraph Registers
        x0[x₀] --> x1[x₁] --> x2[x₂] --> x3[x₃] --> x4[x₄] --> x5[x₅] --> x6[x₆]
    end
    x0 --> XOR((⊕))
    x4 --> XOR
    XOR --> |feedback| x0
...
```mermaid
flowchart TB
    A["📡 Signal Capture<br/>Antenna → ADC → I/Q Samples"] --> B
    
    B["🔍 PSS Detection<br/>For N_ID_2 = 0,1,2:<br/>• Generate PSS reference<br/>• Cross-correlate<br/>• Find peak"] --> C
    
    C["📊 CFO Estimation<br/>• Extract CP region<br/>• Correlate CP with tail<br/>• Extract phase"] --> D
    
    D["🔎 SSS Detection<br/>• Go to SSS position<br/>• Try N_ID_1 = 0...335<br/>• Find best match"] --> E
    
    E["🎯 Calculate PCI<br/>PCI = 3 × N_ID_1 + N_ID_2"]
